...
import { changePost, createNewPost, fetchAllPosts, fetchPostById, updatePost } from "../redux/postSlice";
...
export const EditPost = () => {
    ...
    const handleFormSubmit = async (values) => {
        ...
        // alert("submitting");
        setUploading(true);
        // let url = currentPost.poster;
        let url = '';
        if (selectedImage) {
            // console.log("upload selected image")
        ...
        if (url) {
            ...
            setUploading(false);
            url = currentPost.poster
        }

        if (!uploading) {
            const editAt = new Date();
            const titleSlug = convertToSlug(currentPost.title);
            const newPost = { ...currentPost, titleSlug: titleSlug, poster: url, editAt: editAt };
            // console.log(newPost);
            dispatch(updatePost(newPost)).unwrap().then(() => toast.success("Updated"));
        }
    };
                                ...
                                <div className="col-6">
                                    <Form.Group className="mb-3 col-12" >
                                        ...
                                        <Form.Label htmlFor="poster" className="mt-2">
                                            ...
                                            <img
                                                src={previewImage || currentPost.poster}
